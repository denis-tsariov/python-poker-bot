...
def parse_hand_string(hand_string) -> List[List[str]]:
    ...
    if suited == 's':
        ...
        if rank_map[rank1] == rank_map[rank2]: 
            return [[(rank_map[rank1], suit1), (rank_map[rank2], suit2)] for suit1, suit2 in combinations(suits, 2)]
        return [[(rank_map[rank1], suit1), (rank_map[rank2], suit2)] for suit1, suit2 in permutations(suits, 2)]

hands = poker_hand_percentiles.keys()[0:2]


def treys_format_hand(hand_string) -> Generator[str]: # Wrapper for parse_hand_string that returns a string in the format treys expects
    for hand in parse_hand_string(hand_string):
        yield f"{hand[0][0]}{hand[1][0]}{hand[0][1][0]}{hand[1][1][0]}"

for i in treys_format_hand("AAo"):
    print(i)